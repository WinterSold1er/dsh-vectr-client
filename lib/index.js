...
*/
async function findFreePort(min = TUNNEL_PORT_MIN, max = TUNNEL_PORT_MAX, exclude) {
...
function patchCodebase(metaPath, slug, workspace, options = {}) {
	...
	if (options.instances === void 0 || resolveInstance(options.instances, workspace) === void 0) throw new CodebaseError(409, `workspace "${workspace}" is not registered with a vectr daemon`);
...
async function healTunnelOnce(deps, metaPath, entry) {
	...
	if (localPort === void 0 || !masterUp && await isPortListening("127.0.0.1", localPort, 800)) {
		const fresh = await findFreePort(TUNNEL_PORT_MIN, TUNNEL_PORT_MAX, /* @__PURE__ */ new Set([...inProgressLocalPorts, ...localPort !== void 0 ? [localPort] : []]));
		if (fresh === void 0) {
...
* pragmatic default.
*/
function cleanupStaleTunnelSockets(dir = tmpdir()) {
	let names;
	try {
...
function apply(ctx, config = {}) {
	...
	try {
		...
		for (const entry of loadCodebases(codebasesPath)) if (entry.type === "remote" && entry.status === "up") ensureTunnelUp(deps, codebasesPath, entry).catch((err) => {
	...
	ctx.effect(() => () => {
		...
		handles.clear();
		try {
			for (const entry of loadCodebases(codebasesPath)) if (entry.type === "remote" && (entry.tunnelPid !== void 0 || entry.tunnelCtl !== void 0)) {
				if (entry.tunnelCtl !== void 0 && entry.host !== void 0) try {
					spawn("ssh", [
						"-O",
						"exit",
	...
	ctx.effect(() => webServer.register({
		...
		handler: async (req, res) => {
			...
			if (req.method === "PATCH") {
				...
				try {
					sendJson(res, 200, stripSecret(patchCodebase(codebasesPath, entry.slug, workspace, instances === void 0 ? {} : { instances })));
				} catch (error) {
					if (error instanceof CodebaseError) sendJson(res, error.status, {
						ok: false,
						error: error.message
					});
...
//#endregion
export { Config, DEFAULT_CODEBASES_FILE, DEFAULT_DAEMON_HTTP_TIMEOUT_MS, DEFAULT_DAEMON_TCP_TIMEOUT_MS, DEFAULT_INSTANCES_FILE, DEFAULT_SECRETS_FILE, DEFAULT_SERVER_NAME, DEFAULT_TOOL_CALL_TIMEOUT_MS, DEFAULT_TUNNEL_PROBE_MS, apply, buildCodebaseDeps, ensureTunnelUp, inject, install, installCodebaseConnections, isDaemonAlive, isPortListening, name, probeTunnel, readInstancesFile, registerCodebaseRoutes, registerManagementRoutes, resolveInstance, slugFromPathname };
