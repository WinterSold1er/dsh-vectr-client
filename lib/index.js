...
function hasCodebase(input) {
	if (!input.workspace || typeof input.workspace !== "string" || input.workspace.trim().length === 0) return false;
	if (input.entry !== void 0 && input.entry !== null) return true;
	if (Array.isArray(input.codebases)) return input.codebases.some((item) => item?.workspace === input.workspace);
	return false;
}
/**
