...
const VECTR_GREP_SECTION_TEXT = "Prioritize querying code via vectr tools (mcp__vectr*) when available. If vectr tools are not available, query fails, or yields no results, use the grep tool — not shell grep or rg — to search file contents. Use read on a matched file when you need surrounding context.";
/**
...
*/
function install(ctx, handles, promptFibers, instancesPath, config, agent, codebasesPath, disposed = /* @__PURE__ */ new WeakSet(), inFlightConnections = /* @__PURE__ */ new WeakSet(), codebaseBoundAgents = /* @__PURE__ */ new WeakSet()) {
	...
	if (validEntry !== void 0 && !handles.has(agent) && !inFlightConnections.has(agent)) {
	...
	if (codebasesPath !== void 0 && !codebaseBoundAgents.has(agent)) {
		...
		installCodebaseConnections(ctx, config, agent, codebasesPath, instances, cwd);
	}
}
/**
...
function apply(ctx, config = {}) {
	...
	const inFlightConnections = /* @__PURE__ */ new WeakSet();
	const codebaseBoundAgents = /* @__PURE__ */ new WeakSet();
	for (const agent of ctx.agents.list()) install(ctx, handles, promptFibers, instancesPath, resolved, agent, codebasesPath, disposed, inFlightConnections, codebaseBoundAgents);
