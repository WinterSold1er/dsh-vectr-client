...
import { createCodebase, deleteCodebase, FileCredentialStore, loadCodebases, testCodebase, } from './codebases';
import { DEFAULT_INSTANCES_FILE, DEFAULT_HOST, resolveInstance, readInstancesFile, } from './registry';
 ...
 */
export function install(ctx, handles, instancesPath, config, agent, codebasesPath) {
...
async function installCodebaseConnections(ctx, config, agent, codebasesPath) {
    ...
    for (const entry of entries) {
        if (entry.status !== 'up' || entry.localPort === undefined)
            continue;
        try {
            const conn = startConnection(agent.ctx, {
                transport: 'streamable-http',
 ...
 */
export function registerManagementRoutes(ctx, instancesPath, config) {
 ...
 */
export function buildCodebaseDeps(ctx, secretsPath) {
    ...
    const sshRunner = (args) => spawnRunner('ssh', args);
 ...
 * @param secretsPath - fallback secret file path.
 */
export function registerCodebaseRoutes(ctx, codebasesPath, secretsPath) {
    const webServer = ctx.get('webServer');
    if (webServer === undefined) {
