...
function safeRemove(path, base) {
	...
	if (target !== safeBase && !target.startsWith(`${safeBase}${sep}`)) throw new Error(`vectr-client: refusing to remove ${path}: outside controlled dir ${base}`);
...
function migrateCodebases(metaPath, instances, logger) {
	if (instances === void 0 || Object.keys(instances).length === 0) return {
		changed: false,
		migrated: 0
	};
