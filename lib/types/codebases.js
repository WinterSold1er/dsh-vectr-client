 ...
 */
const takenServerNames = new Set();
/** Clear the in-process server-name registry (test helper). */
export function _resetServerNameRegistry() {
 ...
 * @returns `vectr_<slug>`.
 */
export function deriveServerName(slug) {
    return `vectr_${slug}`;
}
/**
 ...
 */
export function assertSlugAvailable(slug) {
 ...
 */
export function loadCodebases(metaPath) {
    ...
    for (const entry of parsed) {
        if (typeof entry.serverName === 'string')
            takenServerNames.add(entry.serverName);
    }
    return parsed;
}
/**
 ...
 */
export function saveCodebases(metaPath, list) {
 ...
 */
export async function findFreePort(min = 8760, max = 8799) {
 ...
 */
export async function createCodebase(deps, metaPath, spec) {
    ...
    if (spec.auth === 'password') {
        ...
        if (spec.password !== undefined)
            await deps.credStore.set(credentialRef, spec.password);
    }
    ...
    if (installResult.code !== 0) {
        ...
        if (credentialRef !== undefined)
            await deps.credStore.unset(credentialRef);
 ...
 */
function tunnelPidFrom(stdout, _host) {
 ...
 */
export async function deleteCodebase(deps, metaPath, entry) {
    if (entry.type === 'local') {
    ...
    if (entry.credentialRef !== undefined)
        await deps.credStore.unset(entry.credentialRef);
}
/**
 ...
 */
export async function testCodebase(entry) {
    if (entry.localPort === undefined) {
...
export function _removeMeta(metaPath) {
    try {
        rmSync(metaPath);
    }
    catch {
