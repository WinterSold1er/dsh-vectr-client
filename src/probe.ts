 ...
 */
export function isPortListening(host: string, port: number, timeoutMs = DEFAULT_TCP_TIMEOUT_MS): Promise<boolean> {
 ...
 */
export async function diagnoseDaemon(entry: InstanceEntry, deps: ProbeDeps): Promise<AliveDiagnosis> {
  const host = entry.host ?? DEFAULT_HOST
  ...
  if (typeof entry.pid === 'number') {
    try {
      ...
      if (code === 'ESRCH' || code === 'ENOENT') return { alive: false, reason: 'PROCESS_DEAD_ESRCH' }
    ...
    if (!tcpAlive) return { alive: false, reason: 'PORT_CLOSED' }
  }
  ...
  try {
    ...
    if (status === undefined) return { alive: false, reason: 'HTTP_PROBE_UNREACHABLE' }
    return { alive: true }
  } catch {
 ...
 */
export async function isDaemonAlive(entry: InstanceEntry, deps: ProbeDeps): Promise<boolean> {
